...
used_frames = tf.sign(tf.reduce_max(tf.abs(encoder_inputs), axis=2))
...
loss_weights = tf.unstack(tf.cast(used_frames, tf.float16), axis=1)

decoder_weights = tf.Variable(tf.truncated_normal([hidden_size, vocab_size], stddev=0.05, dtype=tf.float16))
decoder_bias = tf.Variable(tf.constant(.1, shape=[vocab_size], dtype=tf.float16))
...
encoder_outputs, encoder_state = tf.nn.dynamic_rnn(
   cell=stacked_lstm, inputs=encoder_inputs, initial_state=initial_state, dtype=tf.float16, sequence_length=encoder_sequence_lengths)
...
loss = tf.contrib.legacy_seq2seq.sequence_loss(logits=preds, targets=labels, weights=loss_weights)
...
with tf.Session() as sess:
    ...
    for i in range(200000):
        ...
        train_op.run(session=sess, feed_dict={encoder_inputs: sequences_batch, raw_labels: labels_batch})
